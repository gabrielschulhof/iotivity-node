...
#define NAPI_CALL(env, theCall, ...)                                       \
  do {                                                                     \
    ...
    if (status != napi_ok) {                                               \
      std::string __resultingStatus = napi_status_to_string((env)) + "\n"; \
      __VA_ARGS__;                                                         \
    }                                                                      \
  } while (0)
...
#define NAPI_CALL_RETURN(env, theCall) NAPI_CALL((env), theCall, return FAIL_STATUS)
...
#define J2C_ASSIGN_PROPERTY_JS_RETURN(env, source, name, destination) \
  J2C_ASSIGN_PROPERTY_JS((env), (source), (name), (destination), return FAIL_STATUS)
...
#define J2C_VALIDATE_VALUE_TYPE_RETURN(env, value, typecheck, message) \
  J2C_VALIDATE_VALUE_TYPE((env), (value), (typecheck), message, return FAIL_STATUS)

#define J2C_VALIDATE_IS_ARRAY_RETURN(env, theValue, nullOk, message) \
  J2C_VALIDATE_IS_ARRAY((env), (theValue), (nullOk), message, return FAIL_STATUS)

#define J2C_GET_STRING_RETURN(env, destination, source, nullOk, name) \
  J2C_GET_STRING((env), (destination), (source), (nullOk), name, return FAIL_STATUS)

#define J2C_ASSIGN_STRING_JS_RETURN(env, destination, source, message) \
  J2C_ASSIGN_STRING_JS((env), (destination), (source), message, return FAIL_STATUS)
