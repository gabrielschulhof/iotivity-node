...
function resourceNotFound( request, resource ) {
	csdk.OCDoResponse( {
...
_.extend( Server.prototype, {
	...
	_createEntityHandler: function( resource ) {
		return _.bind( function( flag, request, url ) {
			...
			if ( eventName === "retrieve" && !resource.active ) {
				resourceNotFound( request, resource );
			} else if ( eventName in { "retrieve": 0, "update": 0, "delete": 0 } && resource ) {
