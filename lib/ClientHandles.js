...
_.extend( ClientHandles.prototype, {
	...
	replace: function( options, handler, operation ) {
		...
		if ( handle ) {
			...
			// The handle may be stale if the transaction was really one-shot, when it appeared to
			// be open-ended (like observing an unobservable resource).
			result = handle.stale ? csdk.OCStackResult.OC_STACK_OK :
				csdk.OCCancel( handle, csdk.OCQualityOfService.OC_HIGH_QOS, null );
		...
		if ( operation !== "remove" ) {
			...
			result = csdk.OCDoResource( handleReceptacle,
				...
				_.bind( function( nativeHandle, response ) {
					setImmediate( _.bindKey( this, "emit", eventName, response ) );
