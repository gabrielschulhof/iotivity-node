...
function cancelHandle( handle ) {
	...
	if ( handle.polling ) {
		if ( handle.timeoutId ) {
			handle.timeoutId = clearTimeout( handle.timeoutId );
		}

		// Even if we have an open GET handle at this point, those can't be cancelled, so we simply
		// overwrite it. When its handler fires it will find that handle.stale is true, and it will
		// short-circuit.
		handle.stale = true;
		handle = null;
	}
	return handle ? csdk.OCCancel( handle, csdk.OCQualityOfService.OC_HIGH_QOS, null ) :
		csdk.OCStackResult.OC_STACK_OK;
}
...
_.extend( ClientHandles.prototype, {
	...
	replace: function( options, handler, operation ) {
		...
		function issueRequest( callback, context ) {
			var result;
			...
			if ( result === csdk.OCStackResult.OC_STACK_OK ) {
				result = handleReceptacle.handle;
			}
			return result;
		}

		function handleRequest( nativeHandle, response ) {
			if ( handle.stale ) {
			...
			if ( operation === "poll" ) {
				handle.handle = null;
				handle.timeoutId = setTimeout( _.bind( function() {
					delete handle.timeoutId;
					handle.handle = issueRequest( handleRequest, this );
					if ( typeof handle.handle !== "object" ) {
		...
		if ( operation !== "remove" ) {
			...
			result = issueRequest( handleRequest, this );
