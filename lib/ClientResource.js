...
function observeParams( resource, query ) {
	return {
...
var ClientResource = function ClientResource( init, forceNew ) {
	...
	_.extend( this,
		...
		_.pick( init, [
			...
			"discoverable", "slow", "secure", "observable", "active", "endpoints"
		] ) );
	...
	this.on( "newListener", function( eventName, listener ) {
		if ( eventName === "update" && !this._private.observer ) {
			...
			result = ClientResource.observe( this, this._private.query );
	...
	this.on( "removeListener", function( eventName ) {
		if ( eventName === "update" &&
			...
			result = handles.replace( observeParams( this, this._private.query ),
...
ClientResource.observe = function( resource, query, fulfill, reject ) {
	var result = true;
	...
	if ( !resource._private.observer ) {
		result = handles.replace( observeParams( resource, query ), updateResource, "add" );
