...
function watchDelete( resource ) {
	...
	if ( listenerCount( resource._private.client, "_resourceslost" ) === 0 ) {
		resource._private.client.on( "devicelost", devicelost );
	}

	resource._private.client.on( "_resourceslost", resourceslost );
	resource._private.resourceslost = resourceslost;
}

function unwatchDelete( resource ) {

	resource._private.client.removeListener( "_resourceslost", resource._private.resourceslost );

	// If nobody cares about "_resourceslost", then we remove the dummy "devicelost" handler too
	if ( listenerCount( resource._private.client, "_resourceslost" ) === 0 ) {
		resource._private.client.removeListener( "devicelost", devicelost );
	}
}
...
var ClientResource = function ClientResource( client, init, forceNew ) {
	var result;
	...
	if ( !( this instanceof ClientResource ) ) {
		return ( ( init instanceof ClientResource ) && !forceNew ) ?
			init : new ClientResource( client, init, forceNew );
	}

	Object.defineProperty( this, "_private", { value: { client: client } } );
