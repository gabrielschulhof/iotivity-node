...
function handlePresenceListener( client ) {
	var callingArguments = Array.prototype.slice.call( arguments, 1 );
	return function( eventName, listener ) {
		var result;

		if ( !client._globalPresenceListener ) {
			client._globalPresenceListener = globalPresenceListener( client );
		}
		...
		if ( ( eventName === "devicefound" || eventName === "devicelost" ) &&
			...
			}, client._globalPresenceListener ]
				.concat( callingArguments ) );
...
var Client = function Client() {
	...
	this
		.on( "newListener", handlePresenceListener( this ) )
		.on( "removeListener", handlePresenceListener( this, "remove" ) );
};
...
function makeResources( client, resources, address, resolve, filterPath, emit ) {
	var resource, index;
	...
	for ( index in resources ) {
		if ( !filterPath || filterPath === resources[ index ].resourcePath ) {
			resource = ClientResource( client, resources[ index ] );
...
function plainFinder( client, prefix, eventName, listener, uri, query, itemMaker ) {
...
function globalPresenceListener( client ) {
return function globalPresenceListener( response ) {
...
_.extend( Client.prototype, {
	...
	create: function( resourceInit, target ) {
		var client = this;
		var options = _.extend( target ? {
		...
		return options.payload instanceof Error ? Promise.reject( options.payload ) :
			oneShotRequest( _.extend( options, {
				...
				createAnswer: function( response ) {
					...
					return ClientResource( client, init );
				}
			} ) );
	},
	retrieve: function( resourceId, query, listener ) {
		...
		return ( ( resourceId instanceof ClientResource ) ?
			...
			Promise.resolve( _.isEqual( resourceId._private.query, query ) ?
				resourceId : ClientResource( client, resourceId, true ) ) :
	...
	findDevices: function( listener ) {
		return plainFinder( this, "findDevices", "devicefound", listener, csdk.OC_RSRVD_DEVICE_URI,
			null, makeDevice );
	},
	findPlatforms: function( listener ) {
		return plainFinder( this, "findPlatforms", "platformfound", listener,
			csdk.OC_RSRVD_PLATFORM_URI, null, payload.makePlatformInfo );
	},
	findResources: function( options, listener ) {
		...
		if ( options.deviceId ) {
			if ( listener ) {
				this.on( "resourcefound", listener );
			}
			return oneShotRequest( _.extend( {
				...
				query: query,
				createAnswer: _.bind( function( response ) {
					return makeResources( this, payload.processGetOicRes( response.payload ),
						response.addr, options._resolve, options.resourcePath, true );
				}, this )
			}, query ? { query: query } : {} ) );
		} else {
			return plainFinder( this, "findResources", "resourcefound", listener,
				csdk.OC_MULTICAST_DISCOVERY_URI, query,
				_.bind( function( discoveryPayload, address ) {
					return makeResources( this,
						payload.processDiscoveryPayload( discoveryPayload ), address, false,
